  ...
  class DisuPutter {
    ...
    public void put() throws IOException {
      ...
      if (sqlTable.isInsert()) {
        ...
        writer.write(")\"");
        boolean hasAutoGenerated = fields.hasAutoGenerated();
        if(hasAutoGenerated){
          writer.write(", Statement.RETURN_GENERATED_KEYS");
        }
        writer.write(");\n");
        ...
        if (hasAutoGenerated) {
          writer.write("        ResultSet rs = pstmt.getGeneratedKeys();\n");
          writer.write("        if (rs.next()) {\n");
          writer.write("          String strKeyValue = rs.getString(1);\n");
          Field field = fields.getAutoGenerated();
          ...
          writer.write("          object.set" + cappedFieldName + "(");
          writer.write("new " + fieldType + "(strKeyValue));\n");
          writer.write("          if (rs.wasNull()) {\n");
          writer.write("            object.set" + cappedFieldName + "(null);\n");
          writer.write("          }\n");
          writer.write("        }\n");
        }
        writer.write(createEndOfMethod2(false));
